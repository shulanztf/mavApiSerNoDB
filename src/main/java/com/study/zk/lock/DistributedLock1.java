...
/**
 * 
 * @ClassName: DistributedLock1
 * @Description:
 * @see http://blog.csdn.net/desilting/article/details/41280869
 * @author: zhaotf
 * @date: 2017年9月17日 下午6:04:15
 */
public class DistributedLock1 implements Watcher {
	private static final Logger logger = Logger
			.getLogger(DistributedLock1.class);
	...
	private static final String CONNECTION_STRING = "192.168.0.121:2181";
	...
	public DistributedLock1(int id) {
		this.threadId = id;
	...
	public static void main(String[] args) {
		for (int i = 0; i < THREAD_NUM; i++) {
			...
			new Thread() {
				...
				public void run() {
					try {
						DistributedLock1 dc = new DistributedLock1(threadId);
	...
	public boolean createPath(String path, String data, boolean needWatch)
		...
		if (zk.exists(path, needWatch) == null) {
			logger.info(LOG_PREFIX_OF_THREAD
					+ "节点创建成功, Path: "
		...
		default: {
			...
			try {
				...
				if (zk.exists(waitPath, false) == null) {
					logger.info(LOG_PREFIX_OF_THREAD + "子节点中，排在我前面的" + waitPath
							+ "已失踪，幸福来得太突然?");
					return checkMinPath();
				} else {
					throw e;
