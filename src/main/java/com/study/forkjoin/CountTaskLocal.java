...
public class CountTaskLocal extends RecursiveTask<Long> {
	...
	public static void main(String[] args) {
		...
		// 生成任务，计算1+2+……+10
		CountTaskLocal ctl = new CountTaskLocal(1L, 100L);
	...
	protected Long compute() {
		...
		if ((end - start) <= THRESHOLD) {
			...
			System.out.println(name + ":合计:" + sum + "(元),start:" + start
					+ ",end:" + end + ",上级:" + this.superName + ",线程:"
					+ Thread.currentThread().getId());
		} else {
			// 任务大于阈值，就由子任务计算
			Long middle = (start + end) / 2;

			Collection<CountTaskLocal> rslt = invokeAll(Arrays.asList(
					new CountTaskLocal(start, middle, name),
					new CountTaskLocal(middle + 1, end, name)));
			...
			System.out.println(name + ":合计:" + sum + ",start:" + start
					+ ",end:" + end + ",(元)子任务,上级:" + this.superName + ",线程:"
					+ Thread.currentThread().getId());
		}
		return sum;
	}
